...
public interface SwTaskMapper {
    ...
    @Insert({
        "insert into sw_task (category, title, ",
        "project, description, ",
        ...
        "#{status,jdbcType=INTEGER}, #{proposerEid,jdbcType=VARCHAR}, ",
    ...
    @Select({
        "select",
        "id, category, title, project, description, level, progress_percent, status, ",
        "proposer_eid, manager_eid, handle_eid, dead_line, resource_ids, start_date, ",
        "end_date, create_time, datetime_lastchange",
        "from sw_task",
        "where id = #{id,jdbcType=INTEGER}"
    })
    @Results({
