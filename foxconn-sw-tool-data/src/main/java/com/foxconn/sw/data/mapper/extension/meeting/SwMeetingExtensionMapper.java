...
public interface SwMeetingExtensionMapper extends SwMeetingMapper {
    ...
    @Select({"<script>",
            ...
            "<if test='data.meetingTitle!=null and data.meetingTitle!=\"\"' >",
            "and s.title like CONCAT('%', #{data.meetingTitle,jdbcType=VARCHAR}, '%') ",
            "</if> ",
            "<if test='data.room!=null and data.room!=\"\"' >",
            "and s.room = #{data.room,jdbcType=VARCHAR} ",
            "</if> ",
            "<if test='data.chairman!=null and data.chairman!=\"\"' >",
            "and EXISTS( select 1 from sw_meeting sm inner join sw_meeting_member smm on sm.id=smm.meeting_id and smm.is_delete=0",
            "where smm.employee_no=#{data.chairman,jdbcType=VARCHAR} and smm.role&amp;1=1 )  ",
            "</if> ",
