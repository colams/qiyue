...
public interface SwWorkReportMapper {
    @SelectProvider(type=SwWorkReportSqlProvider.class, method="countByExample")
    long countByExample(SwWorkReportExample example);

    @DeleteProvider(type=SwWorkReportSqlProvider.class, method="deleteByExample")
    ...
    @Insert({
        "insert into sw_work_report (employee_no, week, ",
        "num, project, project_item, ",
        "days, target, current, ",
        "status, description, ",
        "remark, create_time, ",
        "datetime_lastchange)",
        "values (#{employeeNo,jdbcType=VARCHAR}, #{week,jdbcType=INTEGER}, ",
        "#{num,jdbcType=INTEGER}, #{project,jdbcType=VARCHAR}, #{projectItem,jdbcType=VARCHAR}, ",
    ...
    @Select({
        "select",
        "id, employee_no, week, num, project, project_item, days, target, current, status, ",
